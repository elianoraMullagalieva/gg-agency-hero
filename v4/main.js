...
(function () {
  ...
  /* ============================================================
     ОРКЕСТРОВКА · партии, которые нельзя выразить в CSS
     ============================================================ */

  function initOrchestra() {
    var reduce = window.matchMedia("(prefers-reduced-motion: reduce)").matches;

    /* Счётчики цифр — стартуют в свой такт, вместе с появлением строки */
    var BEAT = 90;
    var counters = [].slice.call(document.querySelectorAll(".figure__value[data-count]"));
    ...
    function runCounters() {
      ...
      counters.forEach(function (el, i) {
        ...
        var dur = 1100 + i * 120;
        var start = performance.now() + BEAT * (8 + i);
    ...
    /* «Дыхание» телефона включаем только после его появления,
       иначе всплытие и колебание накладываются и дёргают кадр. */
    function startFloat() {
      var shot = document.querySelector(".shot");
      if (!shot || reduce) return;
      setTimeout(function () { shot.classList.add("is-in"); }, BEAT * 3.5 + 1150);
    }

    /* Параллакс: телефон и плашки чуть смещаются за курсором.
       Плашки — сильнее, создаётся глубина. */
    function initParallax() {
      var stage = document.querySelector(".stage");
      var shot = document.querySelector(".shot__phone");
      var pills = [].slice.call(document.querySelectorAll(".pill"));
      if (!stage || !shot || reduce) return;
      ...
      function loop() {
        cx += (tx - cx) * 0.06;
        cy += (ty - cy) * 0.06;
        shot.style.setProperty("--px", (cx * 8).toFixed(2) + "px");
        shot.style.setProperty("--py", (cy * 8).toFixed(2) + "px");
        pills.forEach(function (p, i) {
          var k = 14 + i * 5;
          p.style.transform = "translate3d(" + (cx * k).toFixed(2) + "px," +
                              (cy * k).toFixed(2) + "px,0)";
        });
      ...
      // параллакс включаем после того, как плашки доехали
      setTimeout(function () { if (!raf) loop(); }, BEAT * 12 + 800);
    }
